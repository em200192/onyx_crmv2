...
from db_utils import search_errors_db, log_escalation_gsheet
import csv
...
from db_utils import search_errors_db, log_escalation_gsheet, load_all_errors_db
# --- CONFIGURATION ---
st.set_page_config(
...
SALES_KB_PATH = CACHE_DIR / "sales_guide_kb.json" # ADD THIS
PURCHASES_KB_PATH = CACHE_DIR / "purchases_guide_kb.json" # ADD THIS
INVENTORY_KB_PATH = CACHE_DIR / "inventory_guide_kb.json" # ADD THIS
# ...

ESCALATIONS_LOG_PATH = CACHE_DIR / "escalations.csv"
...
def create_triage_chain():
    """
    Creates a lightweight LLM chain to classify the user's initial intent.
    """
    llm = get_llm_model()
    prompt = PromptTemplate(
        template="""
You are a triage agent for a chatbot. Classify the user's message into one of three intents:
1.  `greeting`: For hellos, salutations, and conversational openings.
2.  `vague_problem`: For short, non-specific requests for help or statements of a problem.
3.  `specific_query`: For any message that contains enough detail to be searchable, such as an error number, a specific action, or a question about a feature.

Examples:
- "السلام عليكم" -> {{"intent": "greeting"}}
- "ياهلا" -> {{"intent": "greeting"}}
- "I have an issue" -> {{"intent": "vague_problem"}}
- "ساعدني" -> {{"intent": "vague_problem"}}
- "how to add a bank" -> {{"intent": "specific_query"}}
- "error 1495" -> {{"intent": "specific_query"}}
...
# In app.py
ESCALATION_LOGS_DIR = CACHE_DIR / "escalation_logs"
os.makedirs(ESCALATION_LOGS_DIR, exist_ok=True) # Ensures the directory exists

def _log_escalation_for_review(ticket_id: str, query: str, reason: str):
    """Logs a failed query to Google Sheets and saves the chat history to a JSON file."""
    try:
        # --- MODIFIED: This now logs the ticket to your online Google Sheet ---
        log_escalation_gsheet(ticket_id, query, reason)

        # --- This part remains the same to save the chat history file locally ---
        history_log_path = ESCALATION_LOGS_DIR / f"{ticket_id}.json"
        chat_history = st.session_state.get("chat_history", [])
        with open(history_log_path, "w", encoding="utf-8") as f:
            json.dump(chat_history, f, ensure_ascii=False, indent=2)
        print(f"--- SUCCESS: Saved conversation history for ticket {ticket_id} ---")

    except Exception as e:
        print(f"--- ERROR: Could not log escalation details: {e} ---")
...
def create_guide_router_chain():
    """
    Creates a lightweight LLM chain to route a user's query to the correct user guide.
    """
    llm = get_llm_model()
    prompt = PromptTemplate(
        template="""
You are an expert system router for an ERP application. Your job is to determine which department's user guide is best suited to answer the user's question. Choose ONLY from the options provided.

**Available User Guides:**
- `GL`: General Ledger. For questions about accounting, financial statements, vouchers, chart of accounts, currencies, and banks.
- `Sales`: For questions about customers, sales orders, pricing, quotations, and shipping.
- `Purchases`: For questions about suppliers, purchase orders, receiving goods, and vendor invoices.
- `Inventory`: For questions about items, stock levels, warehouses, inventory transfers, and stock counts.

User's Question: "{query}"

Which user guide is the most relevant? Respond with a single name from the list above (e.g., "GL", "Sales").
""",
        input_variables=["query"],
    )
    return LLMChain(llm=llm, prompt=prompt, verbose=False)
...
def load_all_guides():
    """Loads all guide KBs into a dictionary in session state."""
    st.session_state.guide_kbs = {}
    kb_paths = {
        "GL": GUIDE_KB_PATH,
        "Sales": SALES_KB_PATH,
        "Purchases": PURCHASES_KB_PATH,
        "Inventory": INVENTORY_KB_PATH
    }
    for name, path in kb_paths.items():
        if path.exists():
            try:
                with open(path, "r", encoding="utf-8") as f:
                    st.session_state.guide_kbs[name] = json.load(f)
                    print(f"Successfully loaded {name} guide KB.")
            except Exception as e:
                print(f"Error loading {name} guide KB: {e}")
                st.session_state.guide_kbs[name] = []
        else:
            st.session_state.guide_kbs[name] = []
...
def build_gl_guide_kb(pdf_path: str) -> bool:
    ...
    cache_dir = base_dir / "cache"
    guide_kb_path = cache_dir / "gl_guide_kb.json"
    guide_img_dir = cache_dir / "guide_images" /"gl"
    os.makedirs(guide_img_dir, exist_ok=True)
...
def load_errors_kb() -> bool:
    """Loads the errors KB from ChromaDB into session state."""
    try:
        # 1. Load all records from the live ChromaDB collection
        all_errors = load_all_errors_db()
        st.session_state.gl_errors_kb = all_errors

        # 2. Re-populate the valid message numbers for the router, just like before
        st.session_state.valid_message_numbers = {
            str(p.get('message_number', '')).strip()
            for p in all_errors if str(p.get('message_number', '')).strip()
        }
        print(f"Successfully loaded {len(all_errors)} errors from ChromaDB.")
        return True
    except Exception as e:
        print(f"Error loading errors KB from ChromaDB: {e}")
        st.session_state.gl_errors_kb = []
        st.session_state.valid_message_numbers = set()
        return False
...
def _escalate_and_reset(ticket_id: str,user_query: str, preface: str = None):
    ...
    parts.append(
        f"لقد تم تصعيد طلبك للدعم الفني. يرجى تزويد المستشار برقم الحالة التالي للمتابعة:\n\n"
        f"** رقم الحالة: {ticket_id} **\n\n"
...
@st.cache_resource
def create_query_distiller_chain():
    """
    Creates an LLM chain that refines a conversational query into essential keywords for searching.
    """
    llm = get_llm_model()
    prompt = PromptTemplate(
        template="""You are an expert search query refiner for an ERP system. Your task is to extract the core, essential keywords from the user's conversational query. Remove greetings, filler words, locations, user-specific data (like names or cities), and any detail not related to the system's core functions. The output should be a short, clean search query in Arabic.

Examples:
- User Query: "اريد اضيف سند قبض نقدي لعميل في جده"
  Refined Query: "اضافة سند قبض"
- User Query: "كيف اسوي تقرير ميزان المراجعة لفرع الرياض؟"
  Refined Query: "تقرير ميزان المراجعة"
- User Query: "Hello, can you show me how to define a new bank account please?"
  Refined Query: "تعريف حساب بنكي"
...
def llm_search_guide(query: str, kb: list, k_titles: int = 12, top_m: int = 3):
    """
    Generic LLM retrieval that can search any provided knowledge base list.
    """
    # The 'kb' is now passed in as an argument.
    if not kb:
        return {"answer": "The selected knowledge base is empty."}
...
def search_gl_errors_llm(query: str):
    """
    Searches for an error in the ChromaDB collection.
    This function replaces the old logic of searching a local JSON file.
    """
    try:
        # The search_errors_db function in db_utils is smart.
        # It handles both direct number lookups and semantic text searches.
        search_result = search_errors_db(query)

        if search_result:
            # The result from ChromaDB includes metadata. The best match is the first one.
            best_match_metadata = search_result['metadatas'][0][0]
            return best_match_metadata
        else:
            # If ChromaDB returns nothing, we report that it's not found.
            return {"not_found": True, "reason": "no_match_in_db"}

    except Exception as e:
        print(f"Error during ChromaDB search: {e}")
        return {"not_found": True, "reason": "db_search_error"}
...
def initialize_session_state():
    ...
    if "kb_loaded" not in st.session_state or not st.session_state.kb_loaded:
        print("--- First time setup: Loading all knowledge bases... ---")
        ...
        # This is a placeholder for your legacy KB if you still use it
        load_knowledge_base()

        # Generate embedding matrices once
        # _ensure_kb_matrices()

        # Build the "Did you know?" index once
        build_gl_dyk_index()
    ...
    load_all_guides()
    guide_ok = bool(st.session_state.get("guide_kbs", {}).get("GL"))
...
def handle_text_query(user_query):
    ...
    try:
        raw_response = triage_chain.invoke({"query": user_query})["text"]
        triage_result = _json_only(raw_response)
        intent = triage_result.get("intent", "specific_query")
    except Exception as e:
        print(f"Triage agent failed: {e}")
        intent = "specific_query"  # If the agent fails, proceed with the main logic

    if intent == "greeting":
        reply = "أهلاً بك! أنا مساعدك الذكي لنظام Onyx ERP. كيف يمكنني مساعدتك اليوم؟"
        st.session_state.chat_history.append({"role": "assistant", "content": reply})
        st.rerun()
        return
        ...
        reply = (
            "بالتأكيد يمكنني المساعدة. يرجى تزويدي بالمعلومات التالية:\n"
            "- رقم رسالة الخطأ\n"
            "- أو وصف للمشكلة التي تواجهها\n"
            "- أو قم بتحميل صورة من رسالة الخطأ"
        )
        st.session_state.chat_history.append({"role": "assistant", "content": reply})
        st.rerun()
        return
    print("\n\n=============================================")
    print(f"--- DEBUG: handle_text_query() received query: '{user_query}'")
    import difflib
        ...
        if intent == "follow_up":
            ...
            if followup_intent == "search_guide":
                ...
                with st.spinner("🔎 بالبحث في دليل المستخدم..."):
                    res = llm_search_guide(user_query)
    ...
    print(f"--- DEBUG: The router 'decide_source_smart' chose source = '{source}'")
    print("=============================================\n")
    ...
    if source == "errors":
        ...
        if mentioned_nums and not any(n in valid_nums for n in mentioned_nums):
            print("--- DEBUG: No solution found. Preparing to log escalation. ---")

            date_str = datetime.now(timezone.utc).strftime("%d%m")
            unique_hash = uuid4().hex[:4].upper()
            ticket_id = f"CASE-{date_str}-{unique_hash}"



            _log_escalation_for_review(ticket_id, user_query, "number_not_in_kb")

            _escalate_and_reset(ticket_id, user_query)  # We don't need the user-facing message here anymore.
            return
    ...
    with st.spinner("🤔 أفكر..."):
        ...
        if source == "guide":
            ...
            router_chain = create_guide_router_chain()


            print("--- DEBUG: Distilling user query for guide search ---")
            try:
                distiller_chain = create_query_distiller_chain()
                distilled_query = distiller_chain.invoke({"query": user_query})["text"].strip()

                module_choice = router_chain.invoke({"query": distilled_query})["text"].strip()

                print(f"--- DEBUG: Router chose module: '{module_choice}' ---")

            except Exception as e:
                print(f"Query distiller failed: {e}. Using original query.")
                print(f"Router or distiller agent failed: {e}. Defaulting to GL.")
                distilled_query = user_query
                module_choice = "GL"

            selected_kb = st.session_state.guide_kbs.get(module_choice, [])

            res = llm_search_guide(distilled_query,selected_kb)
            ...
            if "candidates" in res and res["candidates"]:
                if "candidates" in res and res["candidates"]:
                    ...
                    try:
                        ...
                        # Use the distilled_query for the relevance check too
                        ...
                        # LOGIC CHANGE: We no longer give up if is_relevant is false.
                        # We trust that if the search found something, it's worth showing.
                        print(f"  -> Relevance Gate Check: Relevant={is_relevant}. Proceeding to show choices.")
                        st.session_state.pending_choices = res["candidates"]

                    except Exception as e:
                        print(f"Relevance Gate failed: {e}. Falling back to showing choices.")
                        st.session_state.pending_choices = res["candidates"]
            ...
            # Case 1: A matching solution was found.
            if isinstance(sr, dict) and "message_number" in sr:
                # (Your existing logic for handling a found solution is great)
                ...
                if title_like and ratio < 0.95 and not mentioned_nums:
                    ...
                    handle_confirmed_solution(sr)
                    st.rerun()
            else:
                date_str = datetime.now(timezone.utc).strftime("%d%m")
                unique_hash = uuid4().hex[:4].upper()
                ticket_id = f"CASE-{date_str}-{unique_hash}"
                # This now correctly catches the fallback case.
                _log_escalation_for_review(ticket_id, user_query, "not_found_in_kb")
                _escalate_and_reset(ticket_id, user_query)
                return
